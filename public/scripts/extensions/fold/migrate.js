 ...
 * @returns {{from: number, to: number, counts: object, flags: object, retired: string[]}} A report.
 */
export function migrate(fold) {
    ...
    const cast = state[CAST_PATH] && typeof state[CAST_PATH] === 'object' ? state[CAST_PATH] : (state[CAST_PATH] = {});
    const threads = state[THREADS_PATH] && typeof state[THREADS_PATH] === 'object' ? state[THREADS_PATH] : (state[THREADS_PATH] = {});
    ...
    if (from >= V2) {
        ...
        return { from, to: V2, counts, flags, retired };
    }
...
function migrateFacts(state, cast, castTable, counts) {
    ...
    const row = {
        ...owner.entity,
        facts: [owner.entity?.facts, ...rescued].filter(Boolean).join('; ').slice(0, MAX_DETAIL),
    };
    cast[owner.key] = row;
    castTable.set(owner.key, row);
    counts.facts += rescued.length;
}
