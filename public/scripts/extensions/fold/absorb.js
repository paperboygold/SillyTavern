...
import { normalizeItemName, splitItemKey, validateInventory, validateStatus } from './state-table.js';
...
export function absorbStateBlock(messageId, { trackState = true } = {}) {
    ...
    if (canStrip) {
        ...
        message.extra.fold_block = message.mes.slice(stripped.length);
...
export function foldBlockIntoState(fields, windowText) {
    const { items, conditions, context } = classifyBlock(fields);
    ...
    const inventory = validateInventory({ inv: current.inv, deltas: proposed, windowText: gateText });
    // ── A card's health block becomes OWNED marks, on the point-of-view character ──
    //
    // The block is the narrator writing about the protagonist; there is nobody else it could be
    // about, and `who: ''` is exactly how `validateStatus` says so (`state-table.js` `resolveOwner`).
    // What the block cannot say is severity, so these arrive `moderate` — the same default §9 gives
    // migrated conditions, and for the same reason: a card writing "Health: bruised ribs" has stated
    // an affliction and declined to rank it, and inventing a rank from the words would be the
    // enumerated judgement §11 bans.
    const status = validateStatus({
        status: current.marks,
        deltas: conditions.map(flag => ({ who: '', flag, on: true })),
        windowText: gateText,
        cast: entities.load(),
        pov: state.pov(),
    });
    ...
    const rejected = [...inventory.rejected, ...status.rejected];
    ...
    // bytes and add no rows to the audit trail — `deriveState` records a contributor only when
    ...
    return {
        applied: inventory.accepted.length + status.accepted.length,
