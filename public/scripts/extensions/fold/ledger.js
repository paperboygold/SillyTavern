 ...
 *
 * `src/endpoints/fold-ledger.js` is the transport, `ledger-table.js` is the replay algebra, and
 ...
 * rename and file moves, which the existing `fold-traces` store does not — that one keys on the chat
 * FILENAME (`src/endpoints/fold-trace.js:36`), so a renamed chat silently orphans its own traces.
 *
...
const LEDGER_API = '/api/fold-ledger';
...
/** @returns {object} The fold blob, without importing store.js (which would cycle through migrate). */
function foldBlob() {
    const blob = chat_metadata?.fold;
...
export async function hydrate() {
    ...
    try {
        ...
        if (hydrated.damaged) {
            ...
            console.error(`[fold] ledger for campaign ${campaign} has ${hydrated.damaged} unreadable line(s); state is incomplete`);
        }
        ...
        console.error('[fold] ledger hydrate failed; falling back to chat metadata', error);
        return false;
    }
}
...
export async function flush() {
    ...
    try {
        ...
        console.error('[fold] ledger append failed; ops held for retry', error);
        return false;
    } finally {
        flushing = false;
    }
...
/** @returns {number} Ops written locally but not yet acknowledged by the server. */
export function pending() {
    return queue.reduce((n, batch) => n + batch.ops.length, 0);
}
