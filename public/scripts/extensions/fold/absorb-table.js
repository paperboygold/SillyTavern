...
/** Reason kept on a health field, which routes but has a residual worth showing. */
export const HEALTH_PROSE = 'health-prose';
 ...
 *   health    → the status pipeline. `classifyBlock` (`block-parse.js`) already splits these into
 *               `conditions` for `validateStatus`; all that was missing is not ALSO parking the raw
 *               string, which is what made one affliction two rows.
...
export function routeBlockFields(context, table, { turn = 0 } = {}) {
    ...
    for (const [label, value] of context ?? new Map()) {
        ...
        if (kind === 'health') {
            // Already routed by `classifyBlock` into `conditions`, which the caller turns into OWNED
            // marks on the point-of-view character (`absorb.js`, Phase D). Nothing to parse here;
            // the defect was never that it failed to route, it was that it ALSO parked. Kept
            // verbatim all the same, because `splitConditions` drops whole clauses that are nothing
            // but reassurance, and "mild hangover, otherwise uninjured" says something a set of
            // marks cannot.
            shadow.push({ label, reason: HEALTH_PROSE, text: value });
            continue;
        }
        ...
        for (const entry of rejected) {
            ...
            shadow.push({ label, reason: SHADOW, text: byName.get(entry.item) ?? entry.item });
        }
    }
