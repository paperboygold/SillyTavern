...
import { isExposition, nameForms } from './entity-table.js';
 ...
 *
 * `per` rides as an ordinary versioned field: it lets a calendar front tick from elapsed time with
 * no model call, and `tickCalendar` reads it. Field-wise last-write carries it without this merge
...
export function foldThread(table, {
    name, tick = 0, size, kind, aka = '', open = '', detail = '', about = '', per = '',
    ...
    insert_with(table, merge_thread, key, {
        ...
        // A calendar cadence. `tickCalendar` below reads `per`, and `clocks.set` now writes one,
        // until it did, this column was accepted here and supplied by nothing, which is why the
        // calendar engine shipped inert.
        //
        // A sibling column `steps` (named dial segments, the Dungeon World front shape) used to be
        // accepted here too and was removed: no writer ever supplied one, no reader ever asked for
        // one, and ZERO rows carried one across all 21 live chats. `per` earned its keep by getting
        // a writer; `steps` never did, and an accepted-but-unsupplied column is indistinguishable
        // from the oversight `per` was until someone measures it.
 ...
 * @returns {{accepted: number, rejected: object[], fired: object[], byForm: number}} What happened.
 *   `byForm` counts admissions granted by a form of the name other than the one written, see
 *   `entity-table.js` `foldEntities`, which reports the same statistic under the same name.
 */
export function foldTicks(table, observations, { turn = 0, windowText = '', mentioned = null } = {}) {
    ...
    // One key space on both sides of the comparison.
    //
    // This gate used to compare `normalizeThreadName(...).key`: markdown stripped, whitespace
    // collapsed, trailing punctuation gone, a review's `T6` label removed, against report entries
    // that had only been lowercased and trimmed. Two key spaces, one comparison: a report entry
    // written `**the entrance trial**` could never match a name normalised out of its asterisks.
    // `foldEntities` has run its report through `normalizeEntityName` since the coverage rule
    // landed (`entity-table.js`) and this is the same discipline, which is what "symmetric with the
    // entity probe's" below is supposed to mean. `nameForms` for the reason it is applied there:
    // a stake named in two scripts is one stake.
    ...
    for (const observed of Array.isArray(observations) ? observations : []) {
        ...
        //
        // It said "symmetric" and it was an if/else where the entity gate has an OR.
        //
        // `foldEntities` reads: "The report is the floor, not the whole answer… A proposal is valid
        // if the model's report names it OR the window literally contains it." This gate had the
        // report REPLACE the window test rather than stand in front of it, so on the on-screen path
        // the structural test, the one that already gates the whole block path, and the one the
        // "may only advance when the window mentions it" test defends, was unreachable.
        //
        // What that cost, replayed across three live campaigns with each pass's own window
        // reconstructed from its own prompt:
        //
        //   Wuxia World RPG   2026-08-20    8 ticks    1 accepted   7 refused
        //   Raccoon City      2026-08-09   19 ticks    3 accepted  16 refused
        //   Time Stop RPG     2026-08-18    5 ticks    5 accepted   0 refused
        //
        // 32 ticks, 9 accepted, 23 refused. Not a bilingual problem: Raccoon City is English
        // throughout and refuses 16 of 19. The cause is a contradiction between two instructions in
        // one probe. `clocks.js` asks for the dial's NAME to be the outcome, constructed, "the
        // Blight reaches Briarwood", never "dealing with the Blight", and asks for `mentions` to be
        // what the excerpt says, "exactly as written". Prose contains the subject, never the
        // constructed outcome, so at turn 12 the model answered `name: "the Raccoon City outbreak
        // escalates"` against `mentions: ["the Raccoon City outbreak", …]` and the gate demanded the
        // one be in the list of the other. Every honest tick fell into that gap.
        //
        // Restoring the OR admits 20 of the 23 (5 of 7 in Wuxia, 15 of 16 in Raccoon City) and
        // refuses 3, and it introduces nothing: `mentionsDial` is the test already sitting in this
        // file. A dial the report does not name AND the window does not touch is still
        // `not-mentioned`, which is exactly the hallucinated advance this gate was built for.
        //
        // The other half of the fix is at the ask, `clocks.js` now requires a ticked dial's name to
        // appear in `mentions`, through the `oneSpelling` clause `chronicle.js` has carried since
        // the coverage rule landed (`coverage.js`). That is what would close all 23 rather than 20,
        // and it cannot be measured from here without spending a live model call.
        const forms = [parsed.key, observed?.about, observed?.where, observed?.aka]
            .flatMap(raw => nameForms(String(raw ?? '')))
            .map(raw => normalizeThreadName(raw)?.key)
            .filter(Boolean);
        ...
        // Admitted by something other than the dial's name as written, see `foldEntities`, which
        // raises the same statistic for the same reason.
    ...
    return { accepted, rejected, fired, byForm };
}
...
/**
 * Is this thread one the last extraction window actually touched?
 *
 * The set is the model's own coverage report (`coverage.js` `coveredThreads`), the `mentions` the
 * thread probe answered for the previous pass, lowercased. Membership is by KEY, exactly as
 * `reviewableWindow` (`review-table.js`) already tests it, so the review block and the panel agree
 * about which threads are in play instead of each deciding for itself.
 *
 * Aliases are included where the review's test is not, and the asymmetry is deliberate: the review
 * poses a thread by the name it will read back, while this answers "is the story about this row",
 * and a report that names a stake by its other name has named the row. `aliasKeys` is the same
 * one-hop resolution the write path follows.
 *
 * This is coverage, never a substring test of the prose ([ROUTER]), a thread is current because
 * the model said it read about it, not because fold matched a token.
 *
 * @param {object} thread A thread record, ideally one from `threads()` so it carries `key`.
 * @param {Set<string>} covered The reported names.
 * @returns {boolean} True when the report names this thread.
 */
export function isTouchedThread(thread, covered) {
    if (!(covered instanceof Set) || covered.size === 0 || !thread) {
        return false;
    }
    return [thread.key, normalizeThreadName(thread.name)?.key, ...aliasKeys(thread)]
        .map(name => String(name ?? '').toLowerCase().trim())
        .some(name => name && covered.has(name));
}

/**
 * What the story is currently about, and on whose authority it says so.
 *
 * Two different claims, and they must not be merged into one list.
 *
 * `touched` is the MODEL's answer: the threads the last extraction window actually mentioned. It is
 * free (the report is already persisted every pass), it is usually right, and it is a statement
 * about the prose. `pinned` is the PLAYER's answer: one key, set by hand (`state.activeThread`), and
 * it exists precisely for the case where the two disagree, pushing on a thread the narration has
 * not reached yet is exactly when you most want it at the top. Dragon Age's *Make Active*.
 *
 * A renderer that collapses them into "current" throws away which of the two said so, and the two
 * are not interchangeable: one is evidence, the other is intent. They are returned apart, and
 * `pinnedKey` comes back beside `pinned` so a caller can tell "nothing pinned" from "the pinned
 * thread is no longer in the table", the second is worth saying out loud rather than silently
 * showing nothing.
 *
 * @param {object[]} list Threads, as `threads()` returns them.
 * @param {object} [options] Options.
 * @param {string} [options.pinned] The player's pinned key (`state.activeThread()`), or ''.
 * @param {Set<string>} [options.covered] The model's coverage report (`coveredThreads()`).
 * @returns {{pinned: object|null, pinnedKey: string, touched: object[]}} The two answers.
 */
export function currentThreads(list, { pinned = '', covered = null } = {}) {
    const rows = Array.isArray(list) ? list : [];
    const key = String(pinned ?? '').trim();
    return {
        pinnedKey: key,
        pinned: key ? (rows.find(thread => thread?.key === key) ?? null) : null,
        touched: rows.filter(thread => isTouchedThread(thread, covered)),
    };
}
