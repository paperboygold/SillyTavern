 ...
 *
 * This is the ONLY ceiling; a `MAX_COLD_ROWS` constant used to claim otherwise.
 *
 * A `MAX_COLD_ROWS = 2000` sat here describing itself as "how much the cold store may hold before
 * it sheds the oldest rows". Nothing read it. The pruner below is entirely byte-driven, this
 * threshold, then `overBy / 300`: so the store would happily hold ten thousand rows if they fit in
 * the blob, and would shed rows well under two thousand if they did not. The constant did not
 * merely go unused; it documented a policy the code does not implement, which is worse than no
 * documentation. Bytes are the real budget because the blob is what has a hard cap, so the byte
 * rule stays and the row count is gone.
 */
...
registerPruner((overBy) => {
    ...
    if (dropped) {
        commit(COLD_PATH, table);
        console.debug(`[sanguine] cold store shed ${dropped} old row(s) to fit the metadata budget`);
    }
}, PRUNE_ARCHIVE);
