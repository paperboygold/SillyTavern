 ...
 * `src/endpoints/sanguine-ledger.js`; the persistence glue will live in `ledger.js`.
 *
 ...
 *
 * `ev` is last-write-wins (`merge_b`), and last-write **diverges without a clock**,
...
import { insert_with, merge_b, merge_bu, table_entries } from './lib/hash.js';

/**
 * The op kinds this build understands. Anything else is skipped by `effectiveOps`.
 *
 * `amend` was here, and was removed; the reader-compatibility argument was empty.
 *
 * `amend` rewrote an event's summary in place, a byte optimisation over re-sending the row. The
 * rule for retiring an op is that the READER must keep understanding anything a WRITER once
 * produced, because a ledger outlives the build that wrote it. `amend` never cleared that bar: no
 * build in this project's history has ever emitted one (`git log --all -S"op: 'amend'"` is empty),
 * and it appears 0 times in 2,552 on-disk ledger lines across 24 campaigns, against 2,532 `ev` and
 * 20 `forget`. There is no older stream to stay compatible WITH.
 *
 * It was also superseded rather than merely unused, which is the stronger reason. `ev` is
 * last-write on the key, so it is correct for ANY change including a summary rewrite, and both
 * places that reasoned about the choice reached that conclusion independently:
 * `chronicle.js` `saveEvents` diffs the table and emits `ev` for every changed row, and
 * `edit-table.js:154` records that per-change delta editing "needs no new ledger op" for exactly
 * this reason. An op the writer is structurally incapable of producing is not an affordance.
 *
 * `hit` and `rewind` are also unwritten today and are deliberately KEPT, see `replay` and
 * `effectiveOps`. Under the unknown-op policy below, a line this build does not know is skipped and
 * counted, never rejected, so retiring `amend` degrades safely even for a writer outside this repo.
 */
export const OPS = Object.freeze(['ev', 'forget', 'hit', 'rewind']);
 ...
 *
 * Undoing by appending compensating ops would need every op to have an inverse, which `forget` does
 * not (you cannot un-forget without knowing what was forgotten). Truncation needs no
    ...
    for (const op of ops ?? []) {
        ...
        switch (op.op) {
            ...
            case 'hit':
                ...
                //
                // Unwritten today, and kept anyway, because it is a DURABILITY affordance.
                //
                // No writer emits `hit` yet (0 of 2,552 on-disk lines), but the counter it feeds has
                // a live consumer: `chronicle-table.js` `selectEvictions` scores an event's
                // retention as `hits × 10` against recency, so retrieval credit is what stops a
                // repeatedly-useful memory being evicted. That counter currently lives in
                // `chat_metadata` under `chronicle.hits`: inside the 128 KiB blob whose eviction
                // pressure is the entire reason this ledger exists. Deleting `hit` would delete the
                // only durable home for the signal, which is the opposite of dead code: it is the
                // half of a live mechanism that has not been wired up yet. `amend`, by contrast,
                // was superseded rather than unwired, and is gone.
