...
import * as rowsProbe from './rows-probe.js';
import * as audit from './audit.js';

/** Every this many landed extraction passes, the deep audit runs on its own. */
const AUDIT_EVERY = 3;
/** Extraction passes since the last audit run. */
let auditCounter = 0;
import { recordRowsEvent } from './chronicle.js';
...
import { registerStateCard } from './blocks-card.js';
 ...
 */
const PLOT_INJECT_KEY = '4_sanguine_plot';
 ...
 */
const LANG_INJECT_KEY = '3_sanguine_language';
 ...
 */
const VERDICT_INJECT_KEY = '7_sanguine_verdict';
...
function loadSettings() {
    ...
    if (Number.isFinite(stored) && (stored < 1 || stored > 50)) {
        console.warn(`[sanguine] chronicle interval ${stored} is outside the settings control's own range (1-50); `
    ...
    if (Number.isFinite(budget) && budget < MIN_RESPONSE_LENGTH) {
        console.warn(`[sanguine] extraction budget ${budget} is below the ${MIN_RESPONSE_LENGTH} a reasoning `
...
async function adjudicateLastAttempt() {
    ...
    try {
        ...
        console.debug(`[sanguine] ${verdict.explain(outcome.verdict)}`);
        ...
        console.error('[sanguine] adjudication failed', error);
...
export async function interceptGeneration(messages, contextSize, abort, type) {
    ...
    // Authored canon for linked characters who are in the scene. Owns its own injection key, clears
    // itself on every path including the quiet one, and never throws, see `lore.prepare`.
    await lore.prepare({ quiet: type === 'quiet' });
    ...
    try {
        ...
        console.error('[sanguine] language mode injection failed', error);
    }
    ...
    try {
        ...
        console.error('[sanguine] plot guide injection failed', error);
    }
    ...
    if (isStateEnabled()) {
        try {
            setExtensionPrompt(STATE_INJECT_KEY, state.render({ player: playerIdentity(), overrides: loadPromptOverrides() }), extension_prompt_types.IN_CHAT, settings.state.depth, false, extension_prompt_roles.SYSTEM);
        } catch (error) {
            console.error('[sanguine] state injection failed', error);
        }
    }
    ...
    try {
        ...
        console.error('[sanguine] recall injection failed', error);
    }
}
...
function onMessageForBlock(messageId) {
    ...
    try {
        ...
        console.error('[sanguine] failed to advance the turn counter', error);
    }
    try {
        const outcome = absorbStateBlock(Number(messageId), { trackState: isStateEnabled() });
        if (outcome.absorbed) {
            console.debug('[sanguine] absorbed a state block from the reply', outcome);
            panel.render();
        }
    } catch (error) {
        console.error('[sanguine] failed to absorb the state block', error);
    }
}
...
async function onAssistantMessage({ force = false } = {}) {
    ...
    if (result.ok) {
        ...
        // The place table's own recall and prune, which nothing was calling.
        //
        // `places.recall()` and `places.prune()` were written, tested and never invoked, so a place
        // demoted to the cold store could never come home and a stale one was never demoted at all.
        // `cap:places-archived` and `places:recalled` had producers and no path to them, the same
        // class of defect as the world block's Array guard, found by the same triage.
        //
        // The mention set is the places the story is CURRENTLY using: the scene's own location, plus
        // wherever the cast is standing. That is not a substring scan of the prose, RULE 1 forbids
        // that and it is what `cold.covered` exists to avoid, it is the set of place names the
        // model itself wrote this pass, read back off the records it wrote them onto.
        placesTick();
        const applied = result.results?.events;
        console.debug('[sanguine] chronicle extraction', applied);
        await eventSource.emit('sanguine_chronicle_updated', applied);
        // The deep audit rides the extraction cadence, it is a background tidy, not a command.
        //
        // Every `AUDIT_EVERY` landed pass, the audit runs on its own: the exact detectors seed and
        // flag, the model resolves identity/pollution/conservation against the full history, and
        // staleness/capacity land in the Audit tab. No `/fold-` command needed, the record tidies
        // itself. Fire-and-forget; a failing audit must not fail the extraction that succeeded.
        auditCounter++;
        if (auditCounter >= AUDIT_EVERY) {
            auditCounter = 0;
            audit.run({ now: entities.turn() }).catch(() => {});
        }
        return;
    }
    ...
    console.warn(`[sanguine] extraction declined: ${result.reason}`);
}

/**
 * Bring back the places the story has returned to, and demote the ones it has left.
 *
 * Called once per landed extraction pass, which is the same cadence the cast uses
 * (`entities.applyExtraction` recalls before folding and demotes after). Places have no probe of
 * their own yet, so the mention set is assembled from what the pass already wrote.
 *
 * Failure is swallowed deliberately: this is bookkeeping either side of a pass that has already
 * succeeded, and a throw here would turn a good extraction into a visible error.
 */
function placesTick() {
    try {
        const mentioned = new Set();
        const at = state.sceneLocation();
        if (at) {
            mentioned.add(at);
        }
        for (const [, row] of entities.load()) {
            const where = String(row?.place ?? '').trim();
            if (where) {
                mentioned.add(where);
            }
        }
        if (mentioned.size) {
            places.recall(mentioned);
        }
        places.prune();
    } catch (error) {
        console.warn('[sanguine] places tick failed', error);
    }
}

async function renderSettingsUi() {
    const html = await renderExtensionTemplateAsync(MODULE_NAME, 'settings');
    $('#sanguine_container').append(html);
    ...
    $('#sanguine_rpg_mode').prop('checked', isChronicleEnabled() && isStateEnabled()).on('input', async function () {
        await setRpgMode(!!$(this).prop('checked'));
    });
    bindCheckbox('#sanguine_panel', () => settings.panel, v => {
        settings.panel = v;
        panel.setVisible(v);
    ...
    bindCheckbox('#sanguine_enabled', () => settings.enabled, v => { settings.enabled = v; syncSteerBodyClass(); });
    bindCheckbox('#sanguine_steer_enabled', () => settings.steer.enabled, v => { settings.steer.enabled = v; syncSteerBodyClass(); });
    bindCheckbox('#sanguine_steer_remember', () => settings.steer.remember_last, v => { settings.steer.remember_last = v; });
    bindCheckbox('#sanguine_steer_badge', () => settings.steer.show_badge, v => { settings.steer.show_badge = v; });
    bindCheckbox('#sanguine_chronicle_enabled', () => settings.chronicle.enabled, v => { settings.chronicle.enabled = v; });
    bindCheckbox('#sanguine_bilingual', () => settings.bilingual.enabled, v => { settings.bilingual.enabled = v; });
    $('#sanguine_bilingual_language')
    ...
    try {
        ...
        console.warn('[sanguine] connection profiles unavailable; extraction will use the chat model', error);
        $('#sanguine_chronicle_profile_block').hide();
    }
    ...
    const syncProfileWarn = () => {
        $('#sanguine_chronicle_profile_warn').toggle(!settings.chronicle.profile);
    };
    syncProfileWarn();
    $('#sanguine_chronicle_profile').on('change', syncProfileWarn);

    bindCheckbox('#sanguine_state_enabled', () => settings.state.enabled, v => { settings.state.enabled = v; });
    bindCheckbox('#sanguine_state_absorb', () => settings.state.absorb_block, v => { settings.state.absorb_block = v; });
    bindNumber('#sanguine_state_depth', () => settings.state.depth, v => { settings.state.depth = v; });
    bindNumber('#sanguine_chronicle_interval', () => settings.chronicle.interval, v => { settings.chronicle.interval = v; });
    bindNumber('#sanguine_chronicle_window', () => settings.chronicle.window, v => { settings.chronicle.window = v; });
    bindNumber('#sanguine_chronicle_response', () => settings.chronicle.response_length, v => { settings.chronicle.response_length = v; });
    bindNumber('#sanguine_chronicle_topk', () => settings.chronicle.top_k, v => { settings.chronicle.top_k = v; });
    bindCheckbox('#sanguine_chronicle_reasoning', () => settings.chronicle.reasoning, v => { settings.chronicle.reasoning = v; });
    bindNumber('#sanguine_chronicle_depth', () => settings.chronicle.depth, v => { settings.chronicle.depth = v; });

    $('#sanguine_steer_template').val(settings.steer.template).on('input', function () {
        settings.steer.template = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sanguine_steer_template_restore').on('click', function () {
        settings.steer.template = DEFAULT_STEER_TEMPLATE;
        $('#sanguine_steer_template').val(DEFAULT_STEER_TEMPLATE);
        saveSettingsDebounced();
    });
    ...
    $('#sanguine_plot_guide').val(plot.text()).on('input', function () {
        plot.set({ text: String($(this).val()), source: 'manual' });
    });
    $('#sanguine_chronicle_template').val(settings.chronicle.template).on('input', function () {
        settings.chronicle.template = String($(this).val());
        saveSettingsDebounced();
    });
    $('#sanguine_chronicle_extract_now').on('click', async function () {
        const button = $(this);
    ...
    $('#sanguine_state_view').on('click', function () {
        ...
        console.log('[sanguine] state, derived from the live chronicle\n' + lines.join('\n'));
    });

    $('#sanguine_state_clear').on('click', async function () {
    ...
    $('#sanguine_chronicle_view').on('click', function () {
        ...
        console.log(`[sanguine] chronicle (${snapshot.total} events, ${snapshot.live} live on this branch)\n` + (lines.join('\n') || '(empty)'));
    });
}
...
function refreshSettingsUi() {
    const settings = foldSettings();
    $('#sanguine_rpg_mode').prop('checked', isChronicleEnabled() && isStateEnabled());
    $('#sanguine_chronicle_enabled').prop('checked', settings.chronicle.enabled);
    $('#sanguine_state_enabled').prop('checked', settings.state.enabled);
    $('#sanguine_panel').prop('checked', !!settings.panel);
    $('#sanguine_chronicle_interval').val(settings.chronicle.interval);
    $('#sanguine_chronicle_profile').val(settings.chronicle.profile ?? '');
}
...
export async function init() {
    ...
    initPanel({
        ...
        onClose: () => {
            ...
            $('#sanguine_panel').prop('checked', false);
            saveSettingsDebounced();
        },
        onOpen: () => {
            foldSettings().panel = true;
            $('#sanguine_panel').prop('checked', true);
            saveSettingsDebounced();
        },
    ...
    //
    // Phase 1: the state delta no longer rides here. The rows probe (below) is the state source,
    // the model answers rows-ops against the pinned ledger, and the fold projects the equivalent
    // delta into the chronicle. This probe records the narrative events only.
    registerProbe({
        schemaKey: 'events',
        schema: () => chronicle.extractionSchema({ deltaSchema: null }),
        instruction: () => chronicle.extractionInstruction(),
        apply: (fragment, context) => chronicle.applyExtraction(fragment, context),
    });

    // The state probe (Phase 1). The model reads the pinned ledger, every row with its stable
    // opaque id, and answers with operations on ids (`FOLD-CENSUS.md` row 1: identity by id, never
    // by name). The fold (`rows-table.js`) is the only write; the applied ops project into an
    // old-shape delta event so the legacy readers keep folding the same truth from the chronicle.
    registerProbe({
        schemaKey: 'rows',
        schema: () => rowsProbe.schema(),
        instruction: () => rowsProbe.instruction(),
        context: () => rowsProbe.context(),
        apply: (fragment, context) => {
            const outcome = rowsProbe.apply(fragment, context);
            // The projection rides the newest source in the window, like an extraction, so a swipe
            // retracts the state change with the message that caused it.
            const anchor = context?.sources?.[context.sources.length - 1];
            const has = (d) => d && (d.inv?.length || d.vit?.length || d.st?.length);
            if (has(outcome.delta)) {
                recordRowsEvent({
                    summary: 'State updated from the rows fold',
                    delta: outcome.delta,
                    srcKey: anchor?.key,
                    mid: anchor?.mid,
                });
            }
            observe.noteCap('rows-applied', outcome.applied);
            observe.noteCap('rows-refused', outcome.errors.length);
            if (outcome.diffs.length) observe.noteCap('rows-conservation', outcome.diffs.length);
            return outcome;
        },
    });
    ...
    // The flag set is injected on both hooks rather than imported by `entities.js`, for the reason
    // `entities.render` already documents: the table lives in `state.js`, `state.js` imports
    // `entities.js`, and the reverse import would close a cycle. This is the caller that holds both.
    // `context()` tells the extractor who to describe in full; `apply` tells the fold who may store
    // the long form (`entity-table.js` `MAX_DOSSIER`).
    registerProbe({
        schemaKey: 'entities',
        schema: () => entities.schema(),
        instruction: () => entities.instruction(),
        context: () => entities.context({ poi: new Set(state.poiKeys()), at: state.sceneLocation() }),
        apply: (fragment, context) => {
            const outcome = entities.applyExtraction(fragment, { ...context, poi: new Set(state.poiKeys()) });
            state.noteRejections(outcome.rejected);
            return outcome;
        },
    });
    ...
    registerProbe({
        ...
        //
        // `state.pov()` on BOTH sides, and that is load-bearing rather than symmetric-looking.
        // `worldAsks` skips the player's own row, they are not part of the world that moves while
        // they are away, and `applyExtraction` rebuilds the list to resolve the ids the model
        // answered with. Filter one rebuild and not the other and `W3` names a different row coming
        // back than it did going out, so every answer past the player's row lands on its neighbour.
        context: () => world.context(state.pov()),
        apply: (fragment, context) => {
            const outcome = world.applyExtraction(fragment, { ...context, pov: state.pov() });
            state.noteRejections(outcome.rejected);
            return outcome;
        },
    });
    ...
    // The state card under the latest reply (Megumin's block card, adapted): the derived state
    // rendered in-context, with the scene probe's suggested next actions as CYOA buttons.
    registerStateCard();
    ...
    setInterval(() => {
        ...
        if (sync.state === 'syncing'
            ...
            console.warn('[sanguine] an extraction pass has been in flight for >150s; marking it stalled.');
            state.setSync('failed', {
                mid: sync.mid,
                reason: 'stalled',
                detail: 'The last extraction never finished (hung or interrupted). It retries on the next message.',
            });
            // The failed pass left the adaptive cadence wherever it had backed off to; a stall
            // should be recovered from at the floor, not waited out at a longer interval. The very
        ...
        if (sync.state === 'acknowledged') {
            ...
            if (Number.isFinite(mark.mid) && newest > mark.mid) {
                console.debug(`[sanguine] pending extraction has messages past the mark (mid ${mark.mid} -> ${newest}); forcing a catch-up pass.`);
    ...
    const refreshPlotGuideInput = () => {
        const input = $('#sanguine_plot_guide');
    ...
    eventSource.on(event_types.CHAT_LOADED, () => {
        ...
        if (sync.state === 'syncing' || sync.state === 'acknowledged' || sync.state === 'failed') {
            ...
            const newest = (chat ?? []).reduce((last, m, i) => (m?.mes && !m.is_system ? i : last), -1);
            if (Number.isFinite(mark.mid) && newest > mark.mid) {
                console.debug(`[sanguine] chat loaded with messages past the extraction mark (mid ${mark.mid} -> ${newest}); catching up.`);
    ...
    eventSource.on(event_types.WORLD_INFO_ACTIVATED, (entries) => {
        ...
        // The double-injection guard: an entry World Info just fired for must not be repeated by
        // the presence gate. Same window, same reason as the re-selection below.
        lore.reselect(entries);
