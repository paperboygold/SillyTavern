 ...
 */
/**
 * Drop a thread outright, for a hand correction.
 *
 * Distinct from closing one: `settled`/`moot` say the stake RESOLVED and stay legible in the
 * record, which is what the review writes. This says it should never have been a stake at all,
 * the model raised something that was scenery, so it leaves nothing behind to read.
 *
 * The cold store is deliberately not fed here. Demotion exists so an outrun thread stays
 * recallable; a row the player is deleting as wrong is not something recall should surface later.
 *
 * @param {string} key The thread key.
...
export function schema() {
    return {
        ...
        properties: {
            ...
            //
            // This probe's version of the rule was the most expensive of the three.
            //
            // Replaying the 149 traced passes of `Wuxia World RPG - 2026-08-20@19h08m26s160ms.jsonl`
            // through `foldTicks`: 8 ticks proposed, 7 refused as `not-mentioned`, 1 accepted. One
            // dial reached the table in an entire campaign.
            //
            // The reason is structural and it is in this description. A dial's NAME is a phrase the
            // model authors, "the deadline to leave Qīngyún Mén", "Who wrote the note luring Xiǎo Ān
            // north?", and prose never contains it verbatim. Asked for "every name or phrase the
            // excerpt uses for a stake, exactly as written", the model correctly answered with the
            // nouns the excerpt used: `["客院", "山脚茶馆"]` at turn 141, `["Zhào Tiěshān", "arena"]`
            // at turn 116, `["突破了"]` at turn 135. The gate then demanded the dial's name be in
            // that list. The instruction and the gate were asking for two different things, and
            // every tick fell into the gap.
            //
            // All 7 were dials the table did not already hold, which is what makes the ask the
            // right lever: there is no stored label to disagree with, so both strings come from the
            // same call and the model is free to make them agree the moment it is told to.
            mentions: {
                type: 'array',
                description: `Every name or phrase the NEW excerpt actually uses for a stake, the dial's outcome, its subject, or its place, exactly as written: "the Blight", "the residency window", "the Smokewood". One entry per distinct mention. A stake the excerpt does not touch is never listed. A dial you report a tick for HAS been touched, you just read it happen, so its name belongs in this list, worded your way rather than the prose's. ${oneSpelling({ field: 'the tick\'s "name"', thing: 'stake' })}`,
                items: { type: 'string' },
            },
        },
...
export function instruction() {
    return [
        ...
        `List EVERY name or phrase the NEW excerpt actually uses for a stake in "mentions", exactly as written: "the Blight", "the residency window", "the Smokewood". A stake the excerpt does not touch is never listed. This is the coverage proof: only a stake in "mentions" may advance, so every dial you report a tick for must appear in it, the excerpt need not word it the way you do, but you must list it. ${oneSpelling({ field: 'the tick\'s "name"', thing: 'stake' })}`,
    ].join(' ');
}
...
export function applyExtraction(fragment, { turn = 0, windowText = '', sources = [] } = {}) {
    ...
    // Admitted, but not by the dial's name as the tick wrote it, a second form of the name carried
    // it. Raised here rather than in `thread-table.js` because that file is pure and the counters
    // live behind `observe.js`; `foldTicks` returns the number for exactly this line.
    for (let n = 0; n < (ticks.byForm ?? 0); n++) {
    ...
    // Anchored here, RECORDED by the caller, one refusal must not be counted twice.
    //
    // This used to anchor and then call `observe.noteRejections` itself. But `index.js` also calls
    // `state.noteRejections(outcome.rejected)` on what this returns, and `state.noteRejections`
    // forwards to `observe.noteRejections` in turn, so every thread refusal incremented
    // `reject:<reason>` TWICE and wrote two diagnostics-log lines, while the state probe's refusals
    // (which only ever go through `state.js`) were counted once. Any comparison across rules was
    // reading a doubled numerator for this probe and a true one for that one.
    //
    // Neither channel was complete on its own, which is why the repair is not simply deleting one
    // call: the direct call carried `mid` (the log's cause-link jump) and skipped the panel's
    // `state.rejects` table; the caller's call filled `state.rejects` and dropped `mid`. So the
    // anchor stays here, this is the only place that knows which message the pass read, and rides
    // out on the returned rows. `state.noteRejections` spreads the rejection before adding its own
    // `turn`, so `mid` survives the hand-off and both surfaces are finally fed from one count.
    const mid = sources[sources.length - 1]?.mid;
    const rejected = [...ticks.rejected, ...opened.rejected].map(rejection => ({ ...rejection, mid, turn }));
    for (const thread of ticks.fired) {
        ...
        console.debug(`[sanguine] a dial filled: ${thread.name}${thread.about ? `: ${thread.about}` : ''}`);
...
export function tickCalendar({ now, turn = 0 } = {}) {
    ...
    for (const thread of result.fired) {
        ...
        observe.noteCap('clock-fired');
        console.debug(`[sanguine] a calendar front filled: ${thread.name}${thread.about ? `: ${thread.about}` : ''}`);
 ...
 */
export function set(name, { filled, size, kind, about, open, source, where, seen, status, per, turn = 0 } = {}) {
    const table = load();
    ...
    const written = foldThread(table, {
        ...
        // `open`, `source` and `where` are columns `foldThread` has always accepted and this writer
        // silently dropped, so a hand edit to any of them looked like it saved and did nothing.
        open: open ?? current?.open ?? '',
        source: source ?? current?.source ?? '',
        where: where ?? current?.where ?? '',
        // The fourth instance of the class the comment above records.
        //
        // `per` is the calendar cadence `tickCalendar` reads, and it had no writer anywhere: not in
        // this function, not in `clocks.schema()`, not in the panel, not in a slash command. So the
        // one mechanism in fold that moves the world without a model call could only ever reach its
        // anchor branch, and every front in every chat sat still waiting for the story to mention
        // it. The reader shipped; the writer never did.
        //
        // Kept out of `clocks.schema()` deliberately. Strict mode plus `additionalProperties: false`
        // would force the field into every proposed tick, and this codebase has measured what a
        // forced field costs: `merge_vital`'s `max: 0`, where "the omission the description asks for
        // is unexpressible and 0 is what a model sends instead". A cadence has one legitimate
        // author, and it is the person who set the front.
        per: per ?? current?.per ?? '',
