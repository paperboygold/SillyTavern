 ...
 * never answered, so raising the token budget is the fix. A pass that returns `unparseable`
 * produced JSON the parser could not read; that is prompt, schema or model, and no budget fixes
 * it. The reason is recorded verbatim and the panel shows it beside the kind tag, so the
 * difference is visible without reading code.
 */
...
registerPruner((overBy) => {
    ...
    commit(LOG_PATH, table);
    console.debug(`[sanguine] diagnostics log pruned to ${kept} entry/entries to fit the metadata budget`);
}, PRUNE_DIAGNOSTICS);
