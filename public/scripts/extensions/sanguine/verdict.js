...
async function classify(attempt, { window = 6, profileId = '', responseLength = 800 } = {}) {
    ...
    // `ask: false`: the judge reads the record, it never answers the review. Posing the numbered
    // form here shipped it a page of `T3 [3/6] …` ids it is not being asked about, and ran
    // `auditLedger()`'s diagnostics WRITE, on the one fold call the player is blocked waiting for.
    const ledger = state.ledgerBlock({ windowText: attempt, ask: false });
