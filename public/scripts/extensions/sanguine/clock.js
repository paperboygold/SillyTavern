...
/**
 * What the status-block silence means: nothing, a pause, a gap, or a narrator that stopped.
 *
 * One number that meant three things.
 *
 * The footer printed "N turns unreported" for any silence, which is true and unhelpful. MEASURED on
 * the live Raccoon City campaign: the card's narrator emitted blocks through turn 12 and never
 * again, so at turn 96 the panel read "84 turns unreported", an alarm, in the same visual register
 * as a fault, about a feature the chat had stopped using. sanguine extracts from prose; a narrator
 * that stops restating its block costs it nothing at all.
 *
 * The boundary is derived, not dialled.
 *
 * Blocks were arriving up to turn `block`, so `block` IS the span over which the narrator was
 * willing to write them. A silence longer than that whole span is a change of behaviour; a silence
 * shorter than it is a pause inside behaviour that is still going. No constant is invented: a chat
 * whose blocks ran to turn 50 and has been quiet 10 turns is in a gap worth flagging, and one whose
 * blocks stopped at 12 and has been quiet 84 is not.
 *
 * @param {object} clock The stored clock.
 * @returns {{state: 'never'|'current'|'gap'|'stopped', gap: number}} What to say about it.
 */
export function blockReport(clock) {
    const block = Number(clock?.block);
    const seen = Number(clock?.seen);
    // Zero counts as never: chats that ran before `block` defaulted to NaN persisted a literal 0,
    // and reading that as turn zero reports the whole chat as a gap.
    if (!Number.isFinite(block) || block <= 0 || !Number.isFinite(seen)) {
        return { state: 'never', gap: 0 };
    }
    const gap = Math.max(0, seen - block);
    if (gap < 2) {
        return { state: 'current', gap };
    }
    return { state: gap > block ? 'stopped' : 'gap', gap };
}
 ...
 * @returns {object} The new clock, with `accepted` and `reason`.
 */
export function advanceSceneClock(current, { days = 0, minutes = 0, phase = '', clockHour = NaN, clockMinute = NaN, dateChanged = false, date = '' } = {}) {
    const seen = (current?.seen ?? 0) + 1;
    const kept = { ...current, seen };

    // The one number in the extraction reply that had no ceiling on it.
    //
    // `MAX_SKIP` is argued below and applied in `parseSpan` and `skipClock`; nothing imports
    // `skipClock`, so the function that actually moves the clock every pass, this one, read the
    // model's `elapsed_days` with a floor and no bound at all. That was survivable while the clock
    // only drew a header. It stops being survivable the moment the clock is the coordinate that
    // prices a rate, because a runaway day count is then a runaway ledger.
    //
    // The ceiling is deliberately far above real play rather than tuned: the live corpus reaches
    // day 12,985 legitimately, in a campaign where thirty years genuinely pass. What this refuses
    // is 3,650 days claimed in ONE report, which no honest scene makes. Clamped, not rejected,
    // the story still moves, it moves by as much as fold is willing to believe at once, and the
    // refusal is reported so it can be counted rather than inferred.
    const askedDays = Math.max(0, Math.floor(Number(days) || 0));
    const askedMinutes = Math.max(0, Math.floor(Number(minutes) || 0));
    const spanDays = Math.min(askedDays, Math.floor(MAX_SKIP / DAY));
    const spanMinutes = Math.min(askedMinutes, MAX_SKIP);
    const clamped = spanDays !== askedDays || spanMinutes !== askedMinutes;
    const bounded = clamped ? { clamped: true } : {};
    ...
    // …and a day already on record is not a new day.
    //
    // The window a pass reads overlaps the last one's, so the sentence that named a day gets read
    // again and rolls again. `scene.js` records the shape from the Wuxia RP, one overnight sleep
    // reported as a fresh day on three consecutive turns, and answered it with a prompt clause
    // telling the model to count beyond the clock on record. MEASURED on the live Raccoon City
    // campaign, that is not holding: `day` reached 11 across a story spanning at most four.
    //
    // An instruction is not a gate. The probe already reports the day it read and the clock already
    // stores the day it last accepted, so the same string is the same day, equality on fold's own
    // stored value, no month names, no calendar, the same meaning in any language.
    //
    // Only ever fires on a POSITIVE match. An empty `date` beside `date_changed: true` still rolls:
    // the schema permits it, and refusing a boundary fold cannot check would lose real days to
    // protect against imagined ones.
    const said = String(date ?? '').trim();
    const restated = !!said && said === String(current?.date ?? '').trim();
    if (dateChanged && !rolled && !restated) {
        day += 1;
    }
    ...
    const next = {
        ...
        // Kept so the next pass has something to compare against. `advanceClock` (the block path)
        // writes the same field, which is what makes one date on record rather than two.
        ...(said ? { date: said } : {}),
    };

    if (!Number.isFinite(current?.minutes)) {
        return { ...next, ...bounded, moved: seen, accepted: true, reason: 'established' };
    }

    const before = clockScalar(current.day, current.minutes);
    const after = clockScalar(day, minuteOfDay);
    if (after < before) {
        return { ...kept, ...bounded, accepted: false, reason: 'reversed' };
    }
    return {
        ...next,
        ...bounded,
