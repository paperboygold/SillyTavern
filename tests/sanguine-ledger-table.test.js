...
import {
    ...
    replayAt,
    restoreSeq,
} from '../public/scripts/extensions/sanguine/ledger-table.js';
...
describe('ledger-table, the fold', () => {
    ...
    test('a summary rewrite rides `ev`, which is last-write on the key', () => {
        // What `amend` used to do, done by the op that actually gets written. `chronicle.saveEvents`
        // diffs the table and emits a whole-row `ev` for any changed row, which is why `amend` was
        // retired: it was a byte optimisation for a case `ev` already covers correctly.
        const { ops } = effectiveOps([ev(1, 'a', 'before'), ev(2, 'a', 'after')]);
        expect(summaries(replay(ops))).toEqual(['after']);
    });

    test('`amend` is retired: skipped and counted, never applied', () => {
        // The retirement rule, a reader keeps understanding anything a writer once produced, did
        // not protect `amend`, because no build ever produced one (0 of 2,552 on-disk ledger lines,
        // and `git log --all -S"op: 'amend'"` is empty). A line carrying one now degrades under the
        // module's unknown-op policy rather than resurrecting a row `forget` removed.
        const { ops, skipped } = effectiveOps([
            ev(1, 'a', 'x'),
            op(2, { op: 'forget', k: 'a' }),
    ...
    test('`hit` and `rewind` stay declared though nothing writes them', () => {
        // Not an oversight and not dead weight. `hit` is the counter face of a LIVE consumer
        // (`selectEvictions` scores retention by hits); `rewind` is the operator the whole
        // checkpoint-validity guarantee is defined against. Retiring either would remove a
        // mechanism, not an unused name.
        expect(OPS).toContain('hit');
        expect(OPS).toContain('rewind');
        expect(OPS).not.toContain('amend');
    });
