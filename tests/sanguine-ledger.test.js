...
import {
    ...
    resolveHistory,
    writeMeta,
} from '../src/endpoints/sanguine-ledger.js';
...
beforeEach(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'sanguine-ledger-'));
...
describe('sanguine-ledger, append and sequence', () => {
    ...
    test('appending is O(1), it does not rewrite what is already there', () => {
        // The defect this endpoint exists to avoid: `sanguine-trace` reads the whole file and rewrites
        // it on every record (`sanguine-trace.js:45-50`), which is O(file) per append forever.
...
describe('sanguine-ledger, time travel', () => {
...
describe('sanguine-ledger, forking from any point', () => {
...
describe('sanguine-ledger, damage', () => {
...
describe('sanguine-ledger, paths', () => {
    test('a traversal attempt cannot escape the ledger directory', () => {
        const dir = campaignDirectory(dirs, '../../etc');
        expect(dir.startsWith(path.join(root, 'sanguine-ledger'))).toBe(true);
    });
});

describe('sanguine-ledger, export carries the memory with the chat', () => {
