...
import { foldEntities } from '../public/scripts/extensions/sanguine/entity-table.js';
import { PERSON } from '../public/scripts/extensions/sanguine/entity-table.js';
import { foldTicks } from '../public/scripts/extensions/sanguine/thread-table.js';
import { planReview, reviewBlock } from '../public/scripts/extensions/sanguine/review-table.js';
...
describe('coverage by the model\'s report ([ROUTER]), the Star Wars Vesk/Gorak/Weequay window', () => {
    ...
    test('a dial in neither the mentions nor the window is refused, even if a token matches', () => {
        // This assertion was the strict one, and the strictness was an accident of shape.
        //
        // It used to name the dial "The Blight reaches Briarwood" against the window "the blight
        // creeps closer each night" and expect a refusal, but that window NAMES the Blight, so the
        // case it was actually pinning was "a dial the window supports is refused because the report
        // omitted it", which is the opposite of the sibling test above. The entity gate has read
        // report-OR-window since the coverage rule landed, and `thread-table.js` `foldTicks` claimed
        // in its own comment to be "symmetric with the entity probe's" while being an if/else where
        // the entity gate has an OR.
        //
        // Replayed across three live campaigns, that asymmetry refused 23 of 32 proposed ticks,
        // 16 of 19 in Raccoon City, which is English throughout, so it was never a language problem
        // (see `foldTicks` for the per-campaign numbers and the instruction contradiction behind
        // them). Restoring the OR recovers 20 of the 23.
        //
        // So this test is now the dial-shaped copy of the person-shaped one above: NEITHER the
        // report NOR the window, sharing only a content token with the prose. That refusal is what
        // the gate exists for and it still holds.
        const table = new Map();
        const mentioned = new Set(['the residency window']);
        const { accepted, rejected } = foldTicks(table, [
            { name: 'the Nightsong debt comes due', tick: 1 },
        ], { turn: 1, windowText: 'the blight creeps closer each night', mentioned });
        expect(accepted).toBe(0);
        expect(rejected).toEqual([expect.objectContaining({ reason: 'not-mentioned' })]);
    });

    test('a dial the window names is admitted even when the report omitted it', () => {
        // The recovered case, stated positively. `mentions` is the floor; the window test that
        // already gates the whole block path stands behind it, exactly as it does for people.
        const table = new Map();
        const mentioned = new Set(['the residency window']);
        const { accepted, rejected } = foldTicks(table, [
            { name: 'The Blight reaches Briarwood', tick: 1 },
