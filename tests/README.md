...
**The server starts itself.** `playwright.config.js` declares a `webServer` that boots
`node server.js` against an isolated data root at `tests/.st-data` (gitignored), so a test run
can never read or modify real user data. If something is already listening on the port —
your normal `npm start` — that instance is reused instead and nothing new is spawned. Override
the port with `ST_TEST_PORT`.
