...
import {
    ...
    reviewableWindow,
    suspectedPairs,
} from '../public/scripts/extensions/sanguine/review-table.js';
import {
    CLOSED,
    DOOM,
    MOOT,
...
describe('the review block: stable ids on every open line', () => {
    ...
    test('the directed questions are budgeted, most consequential first', () => {
        ...
        // Identity questions lead now that lock contests are gone, two names that may be one thing
        // is the most consequential thing the review can be wrong about.
        expect(built.text).toContain('Q1');
...
describe('the directed money question, §5 fix 1', () => {
     ...
     * while the one purchase was priced five separate times in plain text, including the player's
     * own "I hand over the 120k". `how: bought` is what a model writing that scene says; the row it
     * forgot is the money one, which is the whole of what this question recovers.
     */
    const mid60 = { inv: [{ item: 'darkwood staff', dq: 1, how: BOUGHT }, { item: 'shortsword', dq: 1, how: BOUGHT }] };
    ...
    test('a refused credit counts too, Phase A\'s known cost is this question\'s trigger shape', () => {
        ...
        // payment in the window is exactly what this question surfaces. The refusal carries the
        // model's own proposal, so the `bought` gate is applied to it exactly as to an acceptance.
        expect(creditsWithoutDebit({
            accepted: [],
            refused: [{ item: 'mana-shackle bracers', reason: 'already-recorded', raw: { item: 'mana-shackle bracers', dq: 1, how: BOUGHT } }],
        })).toEqual(['mana-shackle bracers']);
    });

    /*
     * A gift arrives in the same breath as a payment, and the question billed it.
     *
     * Ground truth is the live Wuxia chat of 2026-08-20, durable ledger `cac4153c`:
     *
     ...
     *
     * The jar was the other half of the reward. Reading only DEBITS, the rule saw an item credited
     * with nothing paid, asked what it cost, and the model, handed an unpriced item and a purse
     * holding exactly twenty, answered twenty. The player's whole balance, spent on a gift.
     */
    test('income in the same delta accounts for the turn, a gift is not an unpaid purchase', () => {
        expect(creditsWithoutDebit({
            accepted: [
                { item: 'silver', dq: 20, at: MONEY },
                { item: 'wound medicine', dq: 1 },
            ],
        })).toEqual([]);
    });

    test('and the same jar arriving alone, with no money anywhere, is still not a purchase', () => {
        // The half the money-movement rule above could not reach: at mid 84 the jar is re-narrated
        // on its own, with no silver in the delta at all. Reading only the arithmetic, this is the
        // trigger's textbook shape, an item credited, nothing paid, and it is still a gift. The
        // model's own word is the only thing that separates the two, and it says `given`.
        expect(creditsWithoutDebit({
            accepted: [{ item: 'wound medicine', dq: 1, how: 'given' }],
        })).toEqual([]);
    });
    ...
    test('a restatement is not an acquisition, and a category is not a purchase', () => {
        ...
        expect(creditsWithoutDebit({ accepted: [{ item: 'haidong gumdo', dq: 1, at: 'abilities', how: BOUGHT }] })).toEqual([]);
    });
    ...
    test('the question carries the balance on record', () => {
        ...
        expect(text).toContain('recorded these as BOUGHT and no payment was recorded: darkwood staff, shortsword');
...
/*
 * The `contested locks` suite is gone with the mechanism it tested.
 *
 * Scene-field pinning meant "the narrator may not change this", `setContext` discarded the incoming
 * write: and a contest was the record of that argument. Pinning is meant to PROMOTE a thing, never
 * to stop the model editing it, so the concept was retired rather than repaired. See `panel.js`,
 * where `lockable` used to be, for the measurement that settled it.
 */
...
describe('review-wrong-shape is unreachable for an id fold minted', () => {
    ...
    test('and the prefixes are the five the block documents', () => {
        const { index } = reviewBlock(EVERY_KIND);
        const prefixes = new Set([...index.keys()].map(id => id.replace(/[0-9]+$/, '')));
        // Five, not six: `L` went with the lock contests it prefixed.
        expect([...prefixes].sort()).toEqual(['A', 'M', 'P', 'Q', 'T']);
    });
});
