...
import { DOOM, threads } from '../public/scripts/extensions/sanguine/thread-table.js';
...
describe('rows shape repair: a thread or person is never left at a category place', () => {
    const row = (id, kind, name, place) => ({
        id, kind, name, place, who: '', qty: kind === 'item' ? 1 : 0,
        rank: '', status: 'open', born: 0, seen: 0, dqSinceSet: 0, stated: null,
    });
    ...
    const blob = () => ({
        ...
        state: {
            rows: {
                nextId: 5,
                rows: [
                    row('R1', 'thread', 'correspondence with A.W.', 'carried'),
                    row('R2', 'person', 'A.W.', 'carried'),
                    row('R3', 'item', 'flash drive', 'carried'),
                    row('R4', 'person', 'Kang Min-seo', 'nowon gate'),
                ],
            },
        },
    });

    test('category places are stripped off threads and people, items and literal places survive', () => {
        const fold = blob();
        const report = migrate(fold);
        expect(report.changed).toBe(true);
        const rows = fold.state.rows.rows;
        expect(rows.find(r => r.id === 'R1')).toMatchObject({ kind: 'thread', place: '' });
        expect(rows.find(r => r.id === 'R2')).toMatchObject({ kind: 'person', place: '' });
        expect(rows.find(r => r.id === 'R3')).toMatchObject({ kind: 'item', place: 'carried' });
        expect(rows.find(r => r.id === 'R4')).toMatchObject({ kind: 'person', place: 'nowon gate' });
        expect(fold.state.observed['cap:rows-places']).toBe(2);
    });

    test('a second run finds nothing to heal', () => {
        const fold = blob();
        migrate(fold);
        const after = JSON.parse(JSON.stringify(fold.state.rows));
        const report = migrate(fold);
        expect(report.changed).toBe(false);
        expect(JSON.parse(JSON.stringify(fold.state.rows))).toEqual(after);
    });
});
