...
import { appendTrace, readTrace } from '../src/endpoints/sanguine-trace.js';

/**
 * The sanguine-trace endpoint is the durable sink for fold's prompt->output pairs: one JSONL line
 ...
 */
describe('sanguine-trace endpoint, append/read', () => {
    /** @type {string} */
    let dir;
    ...
    beforeAll(() => {
        dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sanguine-trace-'));
    ...
    test('append writes one JSONL line; read returns it, oldest first', () => {
        ...
        const lines = fs.readFileSync(path.join(dir, 'extensions', 'sanguine-traces', 'chatA.jsonl'), 'utf8')
            .trim().split('\n');
...
/*
 * The client half (`public/scripts/extensions/sanguine/trace.js`) imports `script.js` and cannot be
...
describe('fold trace.js, the client record contract', () => {
    ...
    test('the failure path records the pair too (unparseable replies are evidence)', () => {
        ...
        //
        // Read the whole CALL rather than a fixed slice. The first version took 800 characters from
        // the anchor, which made the gate a measure of how much comment sat inside the call: adding
        // the note explaining why `turn` may not be read here pushed `ok: false` off the end and
        // failed a test whose subject had not changed. Balancing the braces is the same fix
        // `sanguine-no-raw-reject.test.js` already had to make, for the same reason, the shape of
        // the call stops mattering, which is the only way a source-text gate stays about its rule.
        const failAnchor = extract.indexOf('log.note({');
        const open = extract.indexOf('trace.record({', failAnchor);
        expect(open).toBeGreaterThan(failAnchor);

        let depth = 0;
        let end = open;
        for (let at = extract.indexOf('{', open); at < extract.length; at++) {
            if (extract[at] === '{') depth++;
            if (extract[at] === '}') depth--;
            if (depth === 0) { end = at; break; }
        }
        const failureBlock = extract.slice(open, end + 1);
        expect(failureBlock).toContain('ok: false');
        expect(failureBlock).toContain('reason,');
        // The turn is READ on the failure path, never advanced: a pass that produced nothing must
        // not consume a tick, and referencing the pass-scoped `turn` here threw a TDZ error that
        // made the whole failure backstop below it unreachable.
        expect(failureBlock).toContain('entities.turn()');
    });
