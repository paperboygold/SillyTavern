...
describe('the player persona is stated, not inferred', () => {
    ...
    test('the state block leads with a Player line, before anything the probe inferred', () => {
        ...
        expect(state).toContain('const scene = [playerLine, ...context, cast, stakes].filter(Boolean).join(\'\\n\');');
    });
    ...
    test('the Player line survives even when the scene block is otherwise empty', () => {
        ...
        // leaving the model to pick the reader from the people list. The player line is added
        // before context, so it is what the block says when there is nothing else.
        expect(state).toContain('const scene = [playerLine, ...context, cast, stakes]');
    });
