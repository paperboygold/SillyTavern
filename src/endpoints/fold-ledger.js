/**
 * fold-ledger — the durable, append-only event ledger for the fold extension.
 *
 * ── Why this exists ──
 *
 ...
 * (`public/scripts/extensions/fold/store.js`). Measured over the live campaigns, that ceiling binds
 ...
 * ── What it does NOT do ──
 *
 * No edit route and no delete route, for the same reason `fold-trace` has none: a ledger that can be
...
export function ledgerDirectory(userDirectories) {
    const dir = path.join(userDirectories.extensions, 'fold-ledger');
 ...
 *
 * `sanitize` is the same guard `fold-trace` uses on its chat id. Campaign ids are fold-minted
 ...
 * `fold-trace` does (`fold-trace.js:45-50`) — that is O(file) per record and is precisely the cost
...
export async function inlineForExport(userDirectories, raw) {
    ...
    try {
        ...
        const { effectiveOps, replay } = await import('../../public/scripts/extensions/fold/ledger-table.js');
        ...
        console.error('[fold] could not inline ledger for export', error);
        return raw;
    }
}
