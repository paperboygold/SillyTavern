...
| 2 | **fold itself** | `public/scripts/extensions/sanguine/` | this codebase argues with itself; the argument is load-bearing |
...
The fold extension (`public/scripts/extensions/sanguine/`) runs an LLM over every chat window. The whole
...
- `clock.js` carried ~20 English regexes (SPANS, NAMED, MONTHS, DAYS, MARKER_PHASE, parseElapsed
  ...
  moved the clock.
- A `stripGloss` regex was tried to clean a translation-glossed location (`(pinyin, "English")`)
  out of the stored scene location. It assumed the gloss always wears ASCII double-quotes, it
  would mangle a legit place like `The "Red" district (as locals call it)`, do nothing for a
  Korean or Japanese gloss written with 「」 or none, and it read a narrative-derived string fold
  must never judge. The fix is the scene probe's instruction: the model reports the location
  without the gloss, in any language. Reverted; recorded so the next agent does not reach for it.
...
`public/scripts/extensions/sanguine/` and fails if a new hardcoded prose-matching pattern appears. Before
...
- Tests live in `tests/`: `npm run test:unit --prefix tests` for all of them, or append a name to
  ...
  (`tests/package.json`).
- Lint with `npx eslint public/scripts/extensions/sanguine/ tests/ src/`.
